...
def bno08x_node():
    ...
    rospy.loginfo(f"{rospy.get_caller_id()}  bno08x node launched.")
    ...
    rospy.loginfo(f"{rospy.get_caller_id()}  bno08x node finished")

if __name__ == '__main__':
    try:
        bno08x_node()
    except rospy.ROSInterruptException:
        rospy.loginfo(f"{rospy.get_caller_id()}  bno08x node exited with exception.")
